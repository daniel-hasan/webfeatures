...
fold = 5
#c_begin, c_end, c_step = 13,  15, 2
#[local] 1 -3 2.92416  (best c=0.5, g=3.0517578125e-05, rate=1.1476)
...
class LocalWorker(Worker):
    def run_one(self,c,g):
        cmdline = '%s -s 3 -c %s -g %s -v %s %s %s' % \
          (svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        print cmdline
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
            if find(line,"Mean squared error") != -1:
		print("CUSTO: "+str(c))		
                return float(line.split("=")[1].strip())#float(split(line)[-1][0:-1])
